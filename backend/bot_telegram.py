...
def handle_command(command, token, chat_id, cfg):
    cmd = command.strip().lower().split(' ')[0] # just in case of spaces

    if cmd == '/start':
        msg = (
            "🤖 <b>Monitoring Listrik IoT Bot</b> 🤖\n\n"
            ...
            "🧠 <b>/prediksi</b> - AI Prediksi (Coming Soon)\n"
        ...
        handle_command('/start', token, chat_id, cfg)
        ...
        is_stale = False
        latest_ts = None
        p1 = m1['daya'] if m1 else 0
        p2 = m2['daya'] if m2 else 0
        total_p = p1 + p2
        
        if m1 and m1['timestamp']:
            diff = (datetime.now() - m1['timestamp']).total_seconds()
            if diff > 15:
                is_stale = True
        ...
        p1 = m1['daya'] if m1 else 0
        p2 = m2['daya'] if m2 else 0
        msg = f"⚡ <b>TOTAL DAYA SISTEM</b>\n\nTotal saat ini: <b>{p1 + p2} Watt</b>"
        send_message(token, chat_id, msg)

    elif cmd == '/energy':
        m1 = get_latest_data(1)
        m2 = get_latest_data(2)
        e1 = m1['energi'] if m1 else 0
        e2 = m2['energi'] if m2 else 0
        msg = f"🔋 <b>TOTAL ENERGI DIGUNAKAN</b>\n\nEnergi total: <b>{e1 + e2:.2f} kWh</b>"
        send_message(token, chat_id, msg)

    elif cmd == '/mesin1':
        m1 = get_latest_data(1)
        if not m1:
            send_message(token, chat_id, "Belum ada data untuk Mesin 1.")
            return
        msg = (
            "🏭 <b>DETAIL MESIN 1</b>\n\n"
            ...
            f"Arus: {m1['arus']} A\n"
            f"Daya: {m1['daya']} W\n"
            f"Frekuensi: {m1['frekuensi']} Hz\n"
            f"Power Factor: {m1['pf']}\n"
            f"Total Energi: {m1['energi']} kWh"
        )
        send_message(token, chat_id, msg)
        ...
        m2 = get_latest_data(2)
        if not m2:
            send_message(token, chat_id, "Belum ada data untuk Mesin 2.")
            return
        msg = (
            "🏭 <b>DETAIL MESIN 2</b>\n\n"
            ...
            f"Arus: {m2['arus']} A\n"
            f"Daya: {m2['daya']} W\n"
            f"Frekuensi: {m2['frekuensi']} Hz\n"
            f"Power Factor: {m2['pf']}\n"
            f"Total Energi: {m2['energi']} kWh"
        )
        send_message(token, chat_id, msg)

    elif cmd == '/prediksi':
        msg = "🧠 <b>PREDIKSI KONSUMSI</b>\n\nMohon maaf, fitur ini sedang dalam tahap pengembangan (Coming Soon). Pantau update selanjutnya!"
        send_message(token, chat_id, msg)
        ...
        msg = (
            "⚙️ <b>STATUS KUOTA SISTEM</b>\n\n"
            f"Kuota Total: {kuota} kWh\n"
            f"Digunakan: {total_e:.2f} kWh\n"
            f"<b>Sisa Kuota: {sisa:.2f} kWh</b>"
        )
        send_message(token, chat_id, msg)

    elif cmd == '/laporan':
        # Get usage over last 24h
        today = datetime.now()
        ...
        msg = (
            "📈 <b>LAPORAN (24 Jam Terakhir)</b>\n\n"
            f"Total Energi Akumulasi: {total_e:.2f} kWh\n"
            f"Rata-rata Penarikan Daya: {avg_p:.2f} Watt\n"
        )
        send_message(token, chat_id, msg)
    ...
    while True:
        try:
            ...
            if cfg.get('telegram_active', '0') != '1':
                # System bot is disabled! Wait 10 seconds and check again
                time.sleep(10)
                continue
                ...
                if data.get('ok'):
                    for update in data.get('result', []):
                        ...
                        if 'text' in message:
                            ...
                            # SECURITY: MUST MATCH DATABASE CHAT ID
                            if chat_id != my_chat_id:
                                print(f"Menolak pesan dari Unregistered Chat ID: {chat_id}")
                                continue
